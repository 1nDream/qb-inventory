    ...
    for k, item in pairs(table) do
        ...
        for itemName, itemAmount in pairs(item.costs) do
            if QBCore.Shared.Items[itemName] ~= nil then 
                if first then
                    first = false 
                    itemCostLabel = "Needed: " ..itemAmount.. "x " ..QBCore.Shared.Items[itemName]["label"] ..""
                else
...
RegisterNetEvent('inventory:client:CraftItems', function(itemName, itemCosts, amount, toSlot, points)
    ...
    QBCore.Functions.Progressbar("repair_vehicle", "Üretiliyor..", (math.random(2000, 5000) * amount), false, true, {
		...
		StopAnimTask(ped, "mini@repair", "fixing_a_player", 1.0)
        QBCore.Functions.Notify("Failed operation", "error")
        isCrafting = false
	end)
end)
...
RegisterNUICallback("GiveItem1", function(data, cb)
    ...
    for sayi, esc in pairs(closestPlayer) do
        if GetPlayerServerId(esc) == data.player then
            ...
            QBCore.Functions.Notify("There is no one nearby!", "error", 2500)
        end
    end
end)