...
local function AddItem(source, item, amount, slot, info)
	...
	if not itemInfo and not Player.Offline then
		TriggerClientEvent("QBCore:Notify", src, "The item is not valid.", "error")
		return false
	end
...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount)
	...
	if RemoveItem(source, itemData.name, itemAmount, itemData.slot) then
		...
		TriggerClientEvent("QBCore:Notify", source, "You don't own the item!", "error")
		return
	end
end
...
--[[ 				secondInv.label = "Stash-"..id ]]
				secondInv.label = '<i class="fas fa-box-open"></i>'.." Stash: "..'<span style="color:#d80032; font-weight: bold;">'..id.."</span>".." "
...
--[[ 				secondInv.label = "Bagaj - Plaka: "..id ]]
			...
			elseif name == "shop" then
				...
				secondInv.label = '<i class="fas fa-shopping-basket"></i>'.." "..other.label.." - Cash: "..'<span style="color:#64e778;">'..para.."$</span>".." "..''
			...
			elseif name == "otherplayer" then
				...
				if Drops[id] and not Drops[id].isOpen then
					...
					secondInv.name = id
					secondInv.label = '<i class="fas fa-box-open"></i>'.." Ground-"..tostring(id)
		...
		TriggerClientEvent('QBCore:Notify', src, 'You cannot access the inventory at this time.', 'error')
	end
end)
			...
			if toInventory == "player" or toInventory == "hotbar" then
				...
				if toItemData ~= nil then
					...
					if toItemData.amount >= toAmount then
						...
						TriggerClientEvent("qb-inventory:CloseInv", src)
					end
				else
					--None
				end
			...
			elseif QBCore.Shared.SplitStr(toInventory, "-")[1] == "stash" then
				...
				if fromItemData.name == "bag" then
					if stashId == "bag_".. fromItemData.info.stashName then
						...
						TriggerClientEvent('QBCore:Notify', src, "You can't put the same bag in a bag.", 'error')
					else
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot, label)
	...
	if Player == OtherPlayer then return TriggerClientEvent("QBCore:Notify", src, "You can't give yourself stuff", "error") end
	if dist > 2 then return TriggerClientEvent("QBCore:Notify", src, "You're too far away to give stuff", "error") end
	local item = GetItemBySlot(src, slot)
	if not item then TriggerClientEvent("QBCore:Notify", src, "The item you tried to export was not found", "error"); return end
	if item.name ~= name then TriggerClientEvent("QBCore:Notify", src, "Invalid item found, try again", "error"); return end
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			if AddItem(target, item.name, amount, false, item.info) then
				...
				TriggerClientEvent(target, "QBCore:Notify", Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname.." by "..amount.." amount "..item.label.." get it")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, true)
				TriggerClientEvent('inventory:client:ItemBox',src, QBCore.Shared.Items[item.name], "remove")
				TriggerClientEvent(src, "QBCore:Notify", OtherPlayer.PlayerData.charinfo.firstname.." "..OtherPlayer.PlayerData.charinfo.lastname.." by "..amount.." amount "..item.label.." you gave")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
...
QBCore.Commands.Add("resetinv", "Reset Inventory (Admin)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID / stash / plate"}}, true, function(source, args)
	...
	if invType and invId then
		...
		elseif invType == "stash" then
			...
			TriggerClientEvent('QBCore:Notify', source,  "Not valid type.", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', source,  "Gaps must be filled correctly.", "error")
	end
end, "admin")

QBCore.Commands.Add("rob", "Strip the Player", {}, false, function(source, args)
	...
	if Player then
			if itemData then
				...
				if AddItem(id, itemData["name"], amount, false, info) then
					TriggerClientEvent('QBCore:Notify', source, GetPlayerName(id)..' by '..amount..' amount '..itemData["name"]..' give.', "success")
				else
					TriggerClientEvent('QBCore:Notify', source, 'Item cannot be given.', "error")
				end
			else
				TriggerClientEvent('QBCore:Notify', source, 'The item is invalid.', "error")
			end
	else
...
RegisterServerEvent("inventory:server:GiveItem0", function(target, name, amount, slot, isMask)
	...
	if amount == 0 then amount = item.amount end
  	...
  	if amount <= item.amount then
        ...
        TriggerClientEvent('inventory:client:ItemBox',target, QBCore.Shared.Items[item.name], "add")
        TriggerClientEvent('QBCore:Notify', target, Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname.. " by "..amount..' amount '..item.label.." you received!")
        TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, true)
        TriggerClientEvent('inventory:client:ItemBox',src, QBCore.Shared.Items[item.name], "remove")
        ...
        TriggerClientEvent('QBCore:Notify', src, "" ..otherName .. "" .. amount .. " amount  " .. item.label .." give!")
        TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
        TriggerClientEvent('qb-inventory:client:giveAnim', src)
        TriggerClientEvent('qb-inventory:client:giveAnim', target)
      else
